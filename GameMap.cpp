...
void GameMap::add(DisplayObject *object) { objects.push_back(object); }

DisplayObject *GameMap::remove(int index) {
...
void GameMap::update(int currentTime) {
  std::stack<int> outerObjectsIndecies;
  for (int i = 0; i < objects.size(); i++) {
    objects[i]->update(currentTime);
    if (!fullyContains(*objects[i])) {
      outerObjectsIndecies.push(i);
    }
  }

  while (!outerObjectsIndecies.empty()) {
    int i = outerObjectsIndecies.top();
    outerObjectsIndecies.pop();
    objects[i]->resetMovement(currentTime);
  }
}
